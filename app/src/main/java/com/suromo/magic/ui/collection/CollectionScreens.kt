...
import com.suromo.magic.ui.home.widget.PostCard
...
@Composable
fun CollectionFeedWithArticleDetailsScreen(
 ...
 */
@Composable
private fun CollectionScreenWithList(
    uiState: CollectionUiState,
    showTopAppBar: Boolean,
    onRefreshPosts: () -> Unit,
    onErrorDismiss: (Long) -> Unit,
    ...
    hasPostsContent: @Composable (
        uiState: CollectionUiState.HasPosts,
        modifier: Modifier
    ) -> Unit
) {
    Scaffold(
        ...
        LoadingContent(
            empty = when (uiState) {
                is CollectionUiState.HasPosts -> false
                is CollectionUiState.NoPosts -> uiState.isLoading
            },
            ...
            content = {
                when (uiState) {
                    is CollectionUiState.HasPosts -> hasPostsContent(uiState, contentModifier)
                    is CollectionUiState.NoPosts -> {
...
private fun PostList(
    ...
    LazyColumn(
        ...
        item { PostListSection(postsFeed.allPosts, onArticleTapped) }
    }
}
 ...
 */
@Composable
private fun PostListSection(posts: List<Post>, navigateToArticle: (String) -> Unit) {
    Column {
        posts.forEach { post ->
            PostCard(
                post = post,
