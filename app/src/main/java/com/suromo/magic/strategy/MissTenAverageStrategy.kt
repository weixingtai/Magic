        ...
        if (lotteries.isNotEmpty()) {
            for (lottery in lotteries) {
                ...
                for (num in numbers) {
                    ...
                    if (numMap[num.toInt()] == null){
                        numMap[num.toInt()] = 1
                    } else {
                        val count = numMap[num.toInt()]
                        if (count != null) {
                            numMap[num.toInt()] = count + 1
                        }
                    }
                }
                resultMap = numMap.toList().sortedBy { (_, value) -> value}.toMap()
                Log.d("wxt","lottery.longperiod-${lottery.longperiod}:${resultMap}")
...
//        numbers.add(40)
        ...
        var maxValue = resultMap.maxOf { it.value }
        var minValue = resultMap.minOf { it.value }

        var averageValue = (resultMap.maxOf { it.value } + resultMap.minOf { it.value })/2


        for(key in resultMap.keys){
            var value = resultMap[key]
            if (value == maxValue || value == minValue || value == averageValue){
                numbers.add(key)
            }
        }
